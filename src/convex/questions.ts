...
export const edit = mutation({args:{id:v.id("questions"),title:v.string(),body:v.string()},handler:async(ctx,{id,title,body})=>{const me=await viewer(ctx);const q=await ctx.db.get(id);if(!q)throw new Error("Question introuvable.");if(q.authorId!==me.id)throw new Error("Tu ne peux modifier que ta propre question.");const nextTitle=title.trim().slice(0,140);const nextBody=body.trim().slice(0,4000);if(!nextTitle&&!nextBody)throw new Error("La question ne peut pas être vide.");await ctx.db.patch(id,{title:nextTitle||nextBody.slice(0,80),body:nextBody,updatedAt:Date.now()});}});
...
export const editAnswer = mutation({args:{id:v.id("questionAnswers"),body:v.string()},handler:async(ctx,{id,body})=>{const me=await viewer(ctx);const a=await ctx.db.get(id);if(!a)throw new Error("Réponse introuvable.");if(a.authorId!==me.id)throw new Error("Tu ne peux modifier que ta propre réponse.");const text=body.trim().slice(0,4000);if(!text)throw new Error("La réponse ne peut pas être vide.");await ctx.db.patch(id,{body:text,updatedAt:Date.now()});}});
