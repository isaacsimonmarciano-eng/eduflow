...
function QuestionThread({ id, onClose }: { id: Id<"questions">; onClose: () => void }) {
  ...
  return <div className="mt-4 border-t border-border/70 pt-4">
    <div className="mb-3 flex items-center justify-between"><p className="font-bold">Réponses de la classe</p><button onClick={onClose}><X className="size-4" /></button></div>
    <div className="space-y-3">
      {answers?.map((a:any)=><div key={a._id} className="rounded-2xl bg-muted/60 p-3">
        <div className="flex justify-between gap-3"><p className="text-xs font-bold text-muted-foreground">{a.authorName}</p>{a.mine&&<button onClick={()=>removeAnswer({id:a._id})} className="text-muted-foreground hover:text-destructive"><Trash2 className="size-3.5"/></button>}</div>
...
export default function Questions() {
  ...
  return <section>
    ...
    <div className="mt-4 space-y-3">{questions===undefined?<p className="text-muted-foreground">Chargement…</p>:visible.length===0?<div className="rounded-3xl border border-dashed p-10 text-center"><MessageCircleQuestion className="mx-auto mb-3 size-9 text-muted-foreground"/><p className="font-bold">Aucune question ici pour l'instant.</p></div>:visible.map((q:any)=>{const subject=SUBJECTS.find(s=>s.key===q.subjectKey);return <article key={q._id} className="rounded-3xl border bg-card p-5 shadow-sm"><button className="w-full text-left" onClick={()=>setThread(thread===q._id?null:q._id)}><div className="flex items-start justify-between gap-3"><div><span className="rounded-full bg-muted px-2.5 py-1 text-xs font-bold">{subject?.emoji} {q.subjectLabel}</span><h2 className="mt-3 font-display text-lg font-bold">{q.title}</h2><p className="mt-1 whitespace-pre-wrap text-sm text-foreground/80">{q.body}</p></div>{q.mine&&<button onClick={async e=>{e.stopPropagation();if(confirm("Supprimer cette question ?"))await remove({id:q._id});}} className="text-muted-foreground hover:text-destructive"><Trash2 className="size-4"/></button>}</div><div className="mt-4 flex items-center justify-between text-xs text-muted-foreground"><span>Par {q.authorName}</span><span className={`font-bold ${q.answered?"text-emerald-600":"text-orange-600"}`}>{q.answered?`🟢 ${q.answerCount} réponse${q.answerCount>1?"s":""}`:"🟠 Sans réponse"}</span></div></button>{thread===q._id&&<QuestionThread id={q._id} onClose={()=>setThread(null)}/>}</article>})}</div>
  </section>;
}
